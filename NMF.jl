...
# ╔═╡ 742ce930-4f12-49a3-be7f-dfba436bc15e
using JLD2

# ╔═╡ d6027a71-a88b-4bed-9cfe-42954fcc7ff8
using Plots, PlutoUI

# ╔═╡ d5819883-83aa-4deb-851d-c71a5a823ae0
using LoopVectorization

# ╔═╡ 838a32f7-6697-409b-aeaf-f952bb626059
using LinearAlgebra

# ╔═╡ cc90bb0e-d0a8-4424-94b3-6cca916af7ef
begin
	@load "/home/rnarwar/Gits/TestData/IndARD.jld2"
	X = X/1.0f0
	#@load "./PCA.jld2"
	#X = ReducedDimX[:,:]/1.0
end

# ╔═╡ 724f7487-9ccc-4040-bd64-77221f14904f
T = X'X |> x -> x/norm(x)

# ╔═╡ 9506d71b-56a4-4ff7-9aef-41e0ffa0fb7b
ding = mapslices(x -> x |> x->x.-minimum(x) |> x->x/maximum(x), X, dims=[2]);

# ╔═╡ e020c023-46a0-46c4-b106-13e97588dc3c
dong = mapslices(x -> x |> x->x.-minimum(x) |> x->x/maximum(x), X*T, dims=[2]);

# ╔═╡ 9bd7dbe9-73ee-49ef-ac44-ebf27e9586a0
@bind i Slider(-12000:12000)

# ╔═╡ 0a5a38c4-8be9-4318-a966-72d1d44881f8
@bind n Slider(1:19, show_value=true)

# ╔═╡ e8f34b43-a6d6-47e5-814a-d9c2a05282a3
decom = svd(T);

# ╔═╡ 73630ce0-00af-4d9b-88c9-7850f0d29d47
bing = mapslices(x -> x |> x->x.-minimum(x) |> x->x/maximum(x), decom.U, dims=[1]);

# ╔═╡ 043ac63b-fb99-4de2-b88c-7dc7b6d3da1f
begin
	plot(ding[end÷2+i,:]; axis=false,legend=false,ticks=false)
	
	#plot!((T^n)*ding[end÷2+i,:]; axis=false,legend=false,ticks=false)
	
	plot!(dong[end÷2+i,:]; axis=false,legend=false,ticks=false)

	plot!(bing[:,n]; axis=false,legend=false,ticks=false)
end

# ╔═╡ fe173999-8e5f-4e3e-9932-96b9ca2ba5a6
basis = map(1:12) do n
	ding*bing[:,n]
end

# ╔═╡ 6e580a42-a94f-472c-b1e2-a2282b1505aa
maximum.(basis)
