...
# ╔═╡ 80fe509a-82a2-41ae-b813-ba2007422f52
begin
...
# ╔═╡ ae3c6faa-f16c-4f74-9df0-6ff5d82001e2
struct Body
    r::SVector{3, Float64}
    v::SVector{3, Float64}
    m::Float64
...
# ╔═╡ 6e380dac-85c4-4360-a033-5d221616c4c7
struct Bodyn{rType <: AbstractFloat, vType <: AbstractFloat}
    r::Vector{rType}
    v::Vector{vType}
    m::Float64
end

# ╔═╡ cef0c321-c13c-416e-9b5c-5611d305a206
@eval begin 
	Body(r::Vector, 
		v::Vector, 
		m::Number) = 
	Body(SVector{3, Float64}(r), 
		SVector{3, Float64}(v), 
		Float64(m))
end

# ╔═╡ bc56676f-324b-4547-ac45-5429cc76acaa
struct GravitationalSystem{bType <: Body, gType <: Float64}
    bodies::Vector{bType}
    G::gType
end

# ╔═╡ 37aa56da-382d-4f11-9884-3276a485743c
struct Simulation{tType <: Float64}
	system::GravitationalSystem
	tspan::Tuple{tType,tType}
end

# ╔═╡ 86051578-1129-46bb-9972-9dd303e65f81
begin
	bodies = begin
		local n = 2
		[Body([i,j,k],[k,i,j],i*k*j) for i=-n:n,j=-n:n,k=-n:n] |> 
		x->reshape(x, *(size(x)...))
	end
end

# ╔═╡ 4b3640ca-aaa0-4a17-8acd-68597968b7cc
function GravAcc(Bodies)
	n = length(Bodies)
	Rs = map(x->x.r, Bodies)
	Ms = map(x->x.m, Bodies)

	Acc = @SArray zeros(Float64, 3, n)
